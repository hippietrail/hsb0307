...
$.formValidator = 
{
	...
	ShowMessage : function(returnObj)
	{
		...
		{		
			...
			if(intiConfig.errorCallback
			//&&(setting.validateType!="AjaxValidator"||!settings[0].nochange)
			)
				intiConfig.errorCallback(id);//如果有错误则调用定义的回调函数  shankangjie
		...
		{		
			...
			if(intiConfig.okCallback)
				intiConfig.okCallback(id);//如果没有错误则调用定义的回调函数  shankangjie
		}
	},
		...
		jqObjs.each(function(i,elem)
		{
			if(elem.settings[0].bind){
				...
				if(returnObj)
				{
					...
					//&&(returnObj.ajax < 0||!this.settings[0].nochange)
					) {
						isValid = false;
						if (thefirstid == ""){
	...
	AjaxValid : function(returnObj)
	{
		...
		var validategroupid = srcjo.get(0).settings[0].validatorGroup; //校验组id  shankangjie
		
		var initConfig = $.formValidator.getInitConfig(validategroupid);//全局配置  shankangjie
		...
		{	
			...
			success : function(data){
			    ...
			    if(setting.success(data))
			    {
			        ...
			        initConfig.okCallback(id);//没有错误时的回调函数  shankangjie
			    }
			    else
			    {
			        $.formValidator.SetTipState(setting0.tipid,"onError",setting.onerror);
			        setting.isValid = false;
			        initConfig.errorCallback(id);//有错误时的回调函数  shankangjie
			    }
			},
			complete : function(){
	...
	return this.each(function()
	{
		...
		if (srcTag == "INPUT" || srcTag=="TEXTAREA")
		{
			...
			{	
				...
				if(initConfig.okCallback)
				initConfig.okCallback(this.id);//控件获取焦点时清除错误时加上的提示  shankangjie
			});
			...
			{   
				...
				if(returnObj.ajax >= 0 && (this.validoldvalue!=$(this).val()||$(this).val!='')) 
				{
					$.formValidator.SetTipState(tip,"onLoad",settings[returnObj.ajax].onwait);
					$.formValidator.AjaxValid(returnObj);
					//setting.nochange = false;
				}
