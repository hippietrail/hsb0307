...
//验证码
function seccode(width,heigth,classname) {
	var img = 'do.php?ac=seccode&rand='+Math.random();
	var str = '<img id="img_seccode" src="'+img+'" align="absmiddle"';
	if (width!=null) str += ' width="'+width+'" ';
	if (heigth!=null) str += ' heigth="'+heigth+'" ';
...
function resizeImg(id,size) {
	...
	for (i=0; i<theImages.length; i++) {
			...
			if (theImages[i].width > size) {
				...
				theImages[i].style.width = size + 'px';
				if (theImages[i].parentNode.tagName.toLowerCase() != 'a') {
					var zoomDiv = document.createElement('div');
					zoomDiv.id=i;
					theImages[i].parentNode.insertBefore(zoomDiv,theImages[i]);
					zoomDiv.appendChild(theImages[i]);
					zoomDiv.style.position = 'relative';
					
					zoomDiv.style.cursor = 'pointer';
					zoomDiv.style.cssText="margin-bottom:8px";
					...
					theImages[i].parentNode.insertBefore(zoom,theImages[i]);
...
function suboptwo(obj,id){
	...
	var divElement = document.getElementById('notice_number_id');
	if(divElement){

		var aa = new sack();
		aa.method = "POST";
		aa.requestFile = '/pms/note.php?folder=clear_num';
		aa.onCompletion = function(){
	...
	if(document.getElementById(id).style.display == 'none'){
		document.getElementById(id).style.cssText = 'display:block; left:' + tLeft + 'px; top:' + tTop + 'px;';
	}
	else{
...
/**
*   奴隶买卖中判断任务是否选择。 @auther:陈光龙
*/
function checkOneToAll()
{
	var num;
	var result = false;
	var obj = document.slaveform.taskid;
