    ...
    public class sys : DbBase, Isys
    {
        ...
        public void CreatTableData(string TableName)
        {
            string Sql = " CREATE TABLE [dbo].[" + TableName + "](" +
                        ...
                        "[NewsTitleRefer] [nvarchar] (512) NULL ," +
        ...
        public DataTable GetPlatform()//初始化平台参数
        {
            string sql = "SELECT * FROM " + Pre + "sys_parmPlatform";
            return DbHelper.ExecuteTable(CommandType.Text, sql, null);
        }
        #endregion
        ...
        #region Platform
        public int Update_Platform(StringDictionary sys)//更新平台参数
        {
            int result = 0;
            string sql = @"if not exists (select * from fs_sys_parmPlatform WHERE Name =@Name )
                                begin 
                                     INSERT INTO fs_sys_parmPlatform(Name,Value)  VALUES(@Name,@Value) 
                                end
                                else
                                     UPDATE fs_sys_parmPlatform SET Value =@Value  WHERE Name =@Name ";
            foreach (DictionaryEntry de in sys)
            {
                SqlParameter[] parm = GetPlatformParam(de);
                result = DbHelper.ExecuteNonQuery(CommandType.Text, sql, parm);
                if (result == 0)
                {
                    return result;
                }
        ...
        private SqlParameter[] GetPlatformParam(DictionaryEntry sys)
        {
            SqlParameter[] parm = new SqlParameter[2];
            parm[0] = new SqlParameter("@Name", SqlDbType.NVarChar, 500);
            parm[0].Value = sys.Key.ToString();
            parm[1] = new SqlParameter("@Value", SqlDbType.NVarChar, 150);
            parm[1].Value = sys.Value.ToString();
            return parm;
