...
namespace Hg.SQLServerDAL
{
    public class Publish : DbBase, IPublish, IDisposable
    {
        ...
        public DataTable GetLastNews(int topnum, string classid)
        {
            ...
            string Sql = "select top " + topnum + " a.NewsID,a.NewsType,a.isDelPoint, a.NewsTitle,a.sNewsTitle,a.NewsTitleRefer,a.URLaddress,a.Content,a.CreatTime,a.SavePath,a.FileName,a.FileEXName,";
        ...
        public string GetPageTitle(string id, string Str)
        {
            ...
            {
                case "News":
                    Sql = "select top 1 NewsTitleRefer from " + Pre + "news where NewsID=@ID";//NewsTitle
                    break;
                case "Class":
                    Sql = "select top 1 NewsTitleRefer from " + Pre + "news_class where ClassID=@ID";//ClassCName
                    break;
                case "Special":
                    Sql = "select top 1 SpecialCName from " + Pre + "news_special where SpecialID=@ID";
                    break;
