...
namespace Foosun.Web.Install
{
    public partial class step3 : System.Web.UI.Page
    {
        ...
        protected void StartInstall_Click()
        {
            ...
            string s_performType = Request.QueryString["performType"];
            string s_backup = Request.QueryString["backup"];
            string ResultStr="0";
            if (s_performType!=null && s_performType == "new")
            {
                 ResultStr = CreateDataBase(s_datasource, s_userid, s_password, s_initialcatalog, s_tableprefix);
            }
            if (s_performType != null && s_performType == "backup")//lsd添加备份还原
            {
                ResultStr = CreateDataBaseByBackup(s_datasource, s_userid, s_password, s_initialcatalog, s_backup);
            ...
            else if (s_performType == "backup")//备份还原,不用再创建管理员了,也不用再初始化数据了.
            {
                ResultStr = "<span style=\"font-weight:bold;font-size:16px;color:Green;\">恭喜！操作成功!</span><a href=\"finishinstall.aspx?stat=login&s_performType=backup\"> &nbsp; <span style=\"color:blue\">进行下一步操作</span>";
            }
            else
            {
        ...
        private string CreateDataBaseByBackup(string server, string user, string pwd, string dbname, string gbackup)
        {
            //throw new Exception("The method or operation is not implemented.");
            string GetResultStr = "0";
            string s_dbsqlpath = sitedir.TrimEnd('\\') + "\\database\\backup\\";
            string dbbackupName = gbackup;
            try
            {
                if (File.Exists(s_dbsqlpath + dbbackupName))
                {
                    ...
                    DataBaseControl dbhelper = new DataBaseControl();
                    dbhelper.ConnectionString = connStr;
                    dbhelper.DataBaseOfBackupPath = s_dbsqlpath;
                    dbhelper.DataBaseOfBackupName = dbbackupName;
                    dbhelper.DataBaseName = dbname;
                    dbhelper.ReplaceDataBase();

                }
                else
                {
                    GetResultStr = "数据库备份文件不存在!";
                }
            }
            catch (Exception e)
            {
        ...
        protected string CreateDataBase(string server, string user, string pwd, string dbname, string tbpre)
        {
            ...
            try
            {
                if (File.Exists(s_dbsqlpath))
                {
                    ...
                    Foosun.Install.Comm.ExecuteSql(connStr, "master", "IF Not EXISTS (select name from master.dbo.sysdatabases where name = N'" + dbname + "') CREATE DATABASE " + dbname);
