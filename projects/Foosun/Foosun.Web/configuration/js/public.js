...
function showDiv(obj,content)
{
    ...
    document.onclick=function () { 
        if(objDiv.style.display=="")
        {
            ...
            if(IfrRef && IfrRef.style.display == "block")
            {
...
function selectFile(type,obj,height,width)
{
    ...
    switch(arrtype)
        {
           case "video":
	            new  Ajax.Request('../../configuration/system/iframe.aspx?FileType=video',options);
	            break;
            case "file":
	        ...
	        case "user_pic":
	            ...
	            break;
	        case "user_Edit":
	            new  Ajax.Request('../../configuration/system/iframe.aspx?FileType=user_Edit',options);
	            break;
	        case "rulePram":
...
function showMessageRequest(forderName)
{
    ...
    var  options={  
					   method:'get',  
					   parameters:"Type=ShowMessage&userPublishID=" + userPublishID,
					   onComplete:function(transport)
					   { 
							var xmlObj = transport.responseXML;							
							...
							//设置显示内容
						    var messages = "正在发布" + thisPublish + ","
						    + "共需要发布" + maxPublishNumber + "条, "
						    + "已发布" + thisPublisCount + "条, "
						    + "成功" + success + "条, "
						    + "<a href=\"error/GetError.aspx?d=now\" title=\"点击查看错误信息\"><span style=\"color:red;\">失败"+error+"条</span></a>"
						    + "发布成功&nbsp;&nbsp;<a class=\"list_link\" href=\"javascript:history.back();\">返 回</a>&nbsp;&nbsp;<a class=\"list_link\" href=\"../../" + indexname + "\" target=\"_blank\">浏览首页</a>";
	...
	new  Ajax.Request('../../'+forderName+'/publish/AjaxSiteParam.aspx?no-cache='+Math.random(),options);
}