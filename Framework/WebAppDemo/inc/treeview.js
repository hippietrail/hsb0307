...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  if(node.icon!="folder")
  {
    ...
    if(node.icon == "root")
       HTML += "style='clear:none;float:left;' ";
    HTML += "src='"+ this.icons[node.icon].src +"'>";
  }
  HTML += "<A ";
  if(HCN)
...
MzTreeView.prototype.initAll = function(nodes)
{
  for(var i=0;i<nodes.length;i++)
  {
     ...
     span.innerHTML="&nbsp;<a href=javascript:parent.send_sms('"+ user_id +"','"+ escape(nodes[i].text) +"') style='color:#FF0000;text-decoration:underline;'>短信</a>&nbsp;";
     span.innerHTML+="<a href=javascript:parent.send_email('"+ user_id +"','"+ escape(nodes[i].text) +"') style='color:#FF0000;text-decoration:underline;'>邮件</a>";
