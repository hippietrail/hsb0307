...
function CreateXMLParser() {
    var xmlDocus = ["Msxml2.DOMDocument.6.0",
		...
		"Msxml2.DOMDocument",
		"Microsoft.XMLDOM"];
    for (var i = 0; i < xmlDocus.length; i++) {
        try {
            return new ActiveXObject(xmlDocus[i]);
        }
        catch (e) {
            return false;
        }
...
function GetDocument(fileName) {
    var xmlDoc = CreateXMLParser();
    xmlDoc.async = false;
    xmlDoc.load(fileName);

    if (xmlDoc.parseError.errorCode != 0) {
        var myErr = xmlDoc.parseError;
        alert("发生错误： " + myErr.reason);
        return null;
    } else {
