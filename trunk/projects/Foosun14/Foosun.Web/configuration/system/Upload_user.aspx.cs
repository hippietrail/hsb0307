...
public partial class configuration_system_Upload_user : Hg.Web.UI.DialogPage
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (Type == "Upload")
		{
			...
			string str_dirSite = Hg.Config.UIConfig.dirSite;
			string str_dirUserFile = Hg.Config.UIConfig.UserdirFile;
			string str_dirFile = "";
			if (Request.QueryString["FileType"] == "user_discuss")
			{
				str_dirFile = str_dirUserFile + "\\discuss\\" + Hg.Global.Current.UserNum;
			}
			else
			{
				str_dirFile = str_dirUserFile + "\\" + Hg.Global.Current.UserNum;
			}
			...
			string _UserGroupNumber = pd.getUserGroupNumber(Hg.Global.Current.UserNum);
			...
			string[] ReturnStr = tt.Upload(0, 0, radFileType.SelectedValue).Split('$');
			if (ReturnStr[1] == "1")
			{
