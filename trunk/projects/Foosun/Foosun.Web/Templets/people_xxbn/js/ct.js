...
function sogou()
{
	...
	if ((Math.random() * 100) < rate)
		...
		sogou_ad_url = "http://fair.sogou.com/";
	var ct = sogou_param["ct"];
	if (u(ct)) return;
	var path = [{s:"matrix",p:"cpc_matrix"},{s:"context",p:"cpc_ads"},{s:"searchframe",p:"cpc_search"}, {s:"textlink",p:"textlink"}];
	...
	{
		if(typeof(sogou_param[p]) == "string")
		{
			if (p == "ti") {
				e_s = l(sogou_param[p]);
			} else {
