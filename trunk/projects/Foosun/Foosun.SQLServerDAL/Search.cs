    ...
    public class Search : DbBase, ISearch
    {
        ...
        public DataTable SearchGetPage(string DTable,int PageIndex, int PageSize, out int RecordCount, out int PageCount, Foosun.Model.Search si)
        {
            ...
            if (si.tags != null && si.tags != "")
            {
                ...
                if (si.type == "tag")
                {
                    tablesAndWhere += " And Tags Like  CONVERT(Nvarchar(100), @Key) collate CHINESE_PRC_BIN";
                    //tablesAndWhere += " And Tags Like @Key  collate CHINESE_PRC_BIN";
                }
                else if (si.type == "edit")
                {
                    tablesAndWhere += " And editor Like CONVERT(Nvarchar(18), @Key) collate CHINESE_PRC_BIN";
                }
                else if (si.type == "author")
                {
                    tablesAndWhere += " And author Like CONVERT(Nvarchar(100), @Key) collate CHINESE_PRC_BIN";
                }
                else
                {
                    if (DTable != string.Empty)
                    {
                        tablesAndWhere += " And ((Title Like CONVERT(Nvarchar(100), @Key)collate CHINESE_PRC_BIN) Or (Author Like CONVERT(Nvarchar(100), @Key)collate CHINESE_PRC_BIN) Or (Souce Like CONVERT(Nvarchar(100), @Key) collate CHINESE_PRC_BIN) Or (Tags Like CONVERT(Nvarchar(100), @Key) collate CHINESE_PRC_BIN) Or (Content Like CONVERT(Nvarchar(100), @Key) collate CHINESE_PRC_BIN))";
                    }
                    else
                    {
                        tablesAndWhere += " And ((NewsTitle Like CONVERT(Nvarchar(100), @Key) collate CHINESE_PRC_BIN) Or (sNewsTitle Like CONVERT(Nvarchar(100), @Key) collate CHINESE_PRC_BIN) Or (Author Like CONVERT(Nvarchar(100), @Key) collate CHINESE_PRC_BIN) Or (Souce Like CONVERT(Nvarchar(100), @Key) collate CHINESE_PRC_BIN) Or (Tags Like CONVERT(Nvarchar(100), @Key) collate CHINESE_PRC_BIN) Or (Content Like CONVERT(Nvarchar(100), @Key) collate CHINESE_PRC_BIN))";
                    }
                }
            }
            else
