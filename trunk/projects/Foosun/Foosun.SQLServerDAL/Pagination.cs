...
namespace Foosun.SQLServerDAL
{
    public class Pagination : DbBase, IPagination
    {
        ...
        //投稿后台
        protected string[] manage_Contribution_Constr_chicklist ={ "id", "id,ConID,Title,creatTime,UserNum,Source,Tags,Author,ispass,Contrflg", "User_Constr where substring(Contrflg,3,1) = '1' and isadmidel=0 and isCheck=1 and ispass=0 and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by Id desc" };

        protected string[] manage_Contribution_Constr_List ={ "id", "id,ConID,Title,creatTime,Source,Tags,Contrflg,Author,ispass", "User_Constr where substring(Contrflg,3,1) = '1' and isadmidel=0 and isCheck=0 and ispass=0 and SiteID='" + Foosun.Global.Current.SiteID + "'", "order by Id desc" };
        ...
        //新闻栏目
        protected string[] manage_news_class_list ={ "id", "id,ClassID,ClassCName,ClassCNameRefer,ClassEname,ParentID,OrderID,IsURL,IsLock,[Domain],NaviShowtf,isPage,classtemplet,ReadNewsTemplet", "News_Class where isRecyle<>1 and ParentID='0' " + Foosun.Common.Public.getSessionStr() + "", "order by OrderID Desc,id desc" };

        protected string[] manage_news_class_list_1 ={ "id", "id,ClassID,ClassCName,ClassEname,ClassCNameRefer,ParentID,OrderID,IsURL,IsLock,[Domain],NaviShowtf,isPage,classtemplet,ReadNewsTemplet", "News_Class where isRecyle<>1 and ParentID='0' and SiteID=@SiteID", "order by OrderID Desc,id desc" };
        //讨论组
        ...
        public DataTable GetPage(string PageName, int PageIndex, int PageSize, out int RecordCount, out int PageCount, params SQLConditionInfo[] SqlCondition)
        {
            ...
            if (fi == null)
                throw new Exception(" 页面的类型名称被修改, 不是Pagination类原来预定义的页面类名了, 造成没有找到SQL");
            try
            {
