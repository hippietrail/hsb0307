...
Function._validateParameterType = function Function$_validateParameterType(param, expectedType, expectedInteger, expectedDomElement, mayBeNull, paramName) {
    ...
    // Using nodeName to check this is a DOM element for lack of a better test on IE and Safari.
    // This is not entirely foolproof ({nodeName: ''} would seem to be of type Sys.UI.DomElement)
    // but we need something that works cross-browser.
    // Opera and Firefox both have an HTMLElement type of which DOM elements are instances.
...
Type.prototype.registerClass = function Type$registerClass(typeName, baseType, interfaceTypes) {
    ...
    // Saving a type-insensitive index of the registered types on each namespace
...
Date._parseInt = function Date$_parseInt(value) {
    // strip leading zeros so value isn't evaluated as octal.
...
Number._parse = function Number$_parse(value, cultureInfo) {
    // Whidbey currently doesn't account for NativeDigits or DigitSubstitution
    // in parse and format, but might need to consider that in future.
    ...
    // verify number format for consistency with .NET parsing.
    // this will prevent more than one decimal separator, or group separators in the decimal portion.
    var numberFormatRegex = new RegExp("^[+-]?[\\d\\" + grpSeparator + "]*\\" + decSeparator + "?\\d*([eE][+-]?\\d+)?$");
    if (!valueStr.match(numberFormatRegex)) {
        return Number.NaN;
    }

    // strip all group separators.  Can't use regex global replace because nonbreaking spaces won't match in Safari regex.
    valueStr = valueStr.split(grpSeparator).join("");

    // convert to javascript decimal separator.
    ...
    function Sys$_ScriptLoader$_loadScriptsInternal() {
        ...
        if (this._scriptsToLoad && this._scriptsToLoad.length > 0) {
            ...
            if (typeof(nextScript.src) === "string") {
                ...
                // REVIEW: we could cache the reference to the head element
                document.getElementsByTagName('HEAD')[0].appendChild(scriptElement);
                
                // cleanup
                Sys._ScriptLoader._clearScript(scriptElement);

                // If the script element was defined using inline text there is no
                // completion to wait for. We just manually call on ourselves to resume
                // our script loading progress.
                this._loadScriptsInternal();
            }
        }
        else {
...
Sys._ScriptLoaderTask = function Sys$_ScriptLoaderTask(scriptElement, completedCallback) {
    ...
    function Sys$_ScriptLoaderTask$_addScriptElementHandlers() {
        ...
        // FF throws onerror if the script doesn't exist, not loaded. No other browser supports
        // it at this time but if they do, we'll catch it
...
// REVIEW: this used to be in Atlas/XmlDom.js
window.XMLDOM = function window$XMLDOM(markup) {
    ...
    return null;
    // REVIEW/TODO: What about safari, see the old XMLDomParser code in AtlasCompat
}
...
Sys.Services._ProfileService.DefaultWebServicePath = '';
    ...
    function Sys$Services$_ProfileService$load(propertyNames, loadCompletedCallback, failedCallback, userContext) {
        ...
        this._invoke(this._get_path(),
                                        ...
                                        Function.createDelegate(this, this._onLoadFailed), // error(WebServiceError, Context, MethodName)
...
Sys.Services._ProfileService.prototype = {
    ...
    // VSWhidbey 31283: calling load with two of the same property names throws an error, so we strip dups
...
// WORK IN PROGRESS: possible beginings of code to detect altered fields on the properties collection during a save() so we dont send the entire list (doubtful)
//Sys.Services._ProfileService.deepCopy = function(obj) {
//    return Sys.Services._ProfileService._deepCopyInternal(obj, null);
//}
//Sys.Services._ProfileService._deepCopyInternal = function(obj, recursionStack) {
...
//            copy[i] = Sys.Services._ProfileService._deepCopyInternal(obj[i]);
//        }
//        return copy;
//    }
...
//        var copy = {};
//        for(var x in obj) {
//            copy[x] = Sys.Services._ProfileService._deepCopyInternal(obj[x]);            
//        }
//        return copy;
//    }
...
//Sys.Services._ProfileService.areEqual = function(obj, baseline) {
//    return Sys.Services._ProfileService._areEqualInternal(obj, baseline, null);
//}
//Sys.Services._ProfileService._areEqualInternal = function(obj, baseline, recursionStack) {
//	if(obj === baseline) return true;

//	var objType = typeof(obj);

//	// different types, must not be equal
//	if(objType !== typeof(baseline)) return false;

//	if (objType === "function") {
//		// equal if the code is the same
//		return obj.toString() === baseline.toString();
//	}
//	else if(objType === "array") {
//		// recurse into arrays and objects
//		if(obj.length !== baseline.length) {
//			// arrays are different length
//			return false;
//		}
//		
//		// don't recurse into an object we are already recusively evaluating
//		if(recursionStack && Array.indexOf(recursionStack, obj) === -1) {
//		    if(!recursionStack) recursionStack = [];
//		    Array.add(recursionStack, obj);
//    		
//	        for(var i=0; i++; i < obj.length) {
//		        // is an item in the array different?
//                if(!Sys.Services._ProfileService._areEqualInternal(obj[i], baseline[i], recursionStack)) return false;
//	        }

//            // done recursively evaluating the object. We may need to recurse into it again
//            // since there could be another field in the object tree that refers to this object,
//            // and the baseline object may have a different corresponding object. That's ok.
//		    Array.removeAt(recursionStack, recursionStack.length-1);
//		}
//		
//		return true;
//	}
...
//		    for(var x in obj) {
//			    // is an item in the dictionary different?
//			    if(!Sys.Services._ProfileService._areEqualInternal(obj[x], baseline[x], recursionStack)) return false;
//		    }
...
Sys.Serialization.JavaScriptSerializer._serializeWithBuilder = function Sys$Serialization$JavaScriptSerializer$_serializeWithBuilder(object, stringBuilder, sort) {
    ...
    case 'string':
        ...
        // DevDiv 33423: Currently Safari regex doesn't support unicode escapes, when they fix that bug, remove this
...
Sys.Serialization.JavaScriptSerializer.deserialize = function Sys$Serialization$JavaScriptSerializer$deserialize(data) {
    ...
    // serialized dates look like this in memory: "\/Date(123)\/"
    // the regex escaped version of this pattern is: \"\\/Date\(123\)\\/\"
