...
namespace Husb.Security
{
    ...
    {
        #region Fields
        private const string CheckSchemaVersionStoredProcName = "dbo.aspnet_CheckSchemaVersion";
        #endregion
        ...
        private static void PopulateCheckSchemaVersionParamters(Database db, DbCommand cmd, string feature, string version)
        {
            db.AddInParameter(cmd, "Feature", DbType.String, feature);
            db.AddInParameter(cmd, "CompatibleSchemaVersion", DbType.String, version);
            db.AddParameter(cmd, "ReturnValue", DbType.Int32, ParameterDirection.ReturnValue, String.Empty, DataRowVersion.Default, null);
        }

        internal static void CheckSchemaVersion(ProviderBase provider, Database db, string[] features, string version, ref int schemaVersionCheck)
        {
            if (db == null)
            {
            ...
            else if (schemaVersionCheck == 0)
            {
                lock (provider)
                {
                    ...
                    {
                        AddSchemaVersion(provider, db, features, version, ref schemaVersionCheck);
        ...
        private static void AddSchemaVersion(ProviderBase provider, Database db, string[] features, string version, ref int schemaVersionCheck)
        {
            DbCommand cmd = db.GetStoredProcCommand(CheckSchemaVersionStoredProcName);
            int iStatus = 0;

            foreach (string feature in features)
            {
                PopulateCheckSchemaVersionParamters(db, cmd, feature, version);
                db.ExecuteNonQuery(cmd);

                object o = db.GetParameterValue(cmd, "ReturnValue");
                iStatus = ((o != null) ? ((int)o) : -1);
                if (iStatus != 0)
                {
                    schemaVersionCheck = -1;
